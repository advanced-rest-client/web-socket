...
export class WebSocketElement extends LitElement {
  static get properties() {
    return {
       ...
       */
      _reconnectTimer: { type: Number },
      /**
       * Current retry counter
       */
      _retryCounter: { type: Number },
    };
  }
  ...
  /**
   * @return {EventListener} Previously registered handler for `connected` event
   */
  get onconnected() {
  ...
  /**
   * @return {EventListener} Previously registered handler for `disconnected` event
   */
  get ondisconnected() {
  ...
  set ondisconnected(value) {
    if (this._ondisconnected) {
      this.removeEventListener('disconnected', this._ondisconnected);
    }
    if (typeof value !== 'function') {
   ...
   */
  get onretrying() {
    return this._onretryingchanged;
  }
  ...
  set onretrying(value) {
    if (this._onretryingchanged) {
      this.removeEventListener('retrying-changed', this._onretryingchanged);
    }
    if (typeof value !== 'function') {
      this._onretryingchanged = null;
      return;
    }
    this._onretryingchanged = value;
    this.addEventListener('retrying-changed', value);
  }

  /**
   * @return {EventListener} Previously registered handler for `message` event
   */
  get onmessage() {
  ...
  /**
   * @return {EventListener} Previously registered handler for `error` event
   */
  get onerror() {
